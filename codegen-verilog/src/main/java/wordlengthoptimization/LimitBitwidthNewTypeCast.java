 ...
 * @author fs
 */
public class LimitBitwidthNewTypeCast implements OperationVisitor {
  ...
  public LimitBitwidthNewTypeCast(int maxWordlength, int minFractionlength) {
  ...
  public void visit(Operation op) {
    ...
    if (fp.getBitsize() > maxWordlength) {
      ...
      /* we want to cut off lowest value bits, so precision must also be reduced */
      fp.restrictBitwidth(maxWordlength);
      cutOffs++;
    }
  ...
  public void visit(Divide op) {
    visit((Operation) op);
    /* we have only 32 bit dividers */
 /*  FixedPoint type = (FixedPoint) op.getType();
   if (type.getBitsize() < 32)  {
     /* increase precision *
     type.setFractionlength(type.getFractionlength() + (32 - type.getBitsize()));
   }
   if (type.getBitsize() > 32) {
     /* decrease precision *
     type.setFractionlength(Math.max(0, type.getFractionlength()-(type.getBitsize() -32)));
   }
   type.setBitsize(32);
   */
  }
