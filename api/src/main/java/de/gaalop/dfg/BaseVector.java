...
/**
 * This class represents a base vector of the algebra. The base vectors are usually indexed from 1
 * to n where n is the dimension of the algebra. In order to support other base vectors, e.g. for
 * the conformal geometric algebra with e<sub>0</sub> or e<sub>&#8734;</sub>, the index is stored as
 * a string.
 * <p/>
 * Base vectors are usually named e<sub>1</sub> to e<sub>n</sub>.
 */
public final class BaseVector implements Expression {

  /** We use a string in order to support indices like einf. */
  private final String index;

  /**
   * Constructs a new Base Vector with the given index. The index is not restricted to a subset of
   * values but should not be negative.
   * 
  ...
  /**
   * Tries to estimate the ordering of the given base vector. For vectors with integer index, the
   * output is the integer itself. For string representations like e_0 or e_inf of the conformal
   * geometric algebra, the corresponding ordering is used. Other cases are not supported currently. <br>
   ...
   * @return True if this and the other object are of the same class and the base vector index of
   * this and the other object are equal.
   */
  @Override
