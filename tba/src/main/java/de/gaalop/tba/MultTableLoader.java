...
public class MultTableLoader {
    ...
    private void loadProducts(UseAlgebra useAlgebra, String filename_Products, boolean useAsRessource) throws IOException {
        ...
        if (useAsRessource) {
            resourceAsStream = AlStrategy.class.getResourceAsStream(filename_Products);
        } else {
        ...
        while (d.ready()) {
            ...
            useAlgebra.getTableInner().setProduct(index0, index1, Parser.parseMultivector(rest.substring(0,rest.indexOf(';'))));
            rest = rest.substring(rest.indexOf(';')+1);
            useAlgebra.getTableOuter().setProduct(index0, index1, Parser.parseMultivector(rest.substring(0,rest.indexOf(';'))));
            rest = rest.substring(rest.indexOf(';')+1);
            useAlgebra.getTableGeo().setProduct(index0, index1, Parser.parseMultivector(rest));
